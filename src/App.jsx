...
function Dashboard({ user, setUser, token, onLogout }) {
  ...
  const [txHistory, setTxHistory] = useState(user.history || []);
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
        {tab==="market"&&(
          <div style={{padding:14}}>
            ...
            {filtered.map(c=>(
              <div key={c.id} onClick={()=>setTradeCrypto(c)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
        ...
        {tab==="portfolio"&&(
          <div style={{padding:14}}>
            ...
            <div style={{fontWeight:700,fontSize:12,color:C.muted,marginBottom:8}}>Holdings</div>
            {portfolio.length===0?(
              <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:28,textAlign:"center",color:C.muted}}>
                <div style={{fontSize:28,marginBottom:8}}>📊</div>
                <div>No positions yet. Buy crypto to get started.</div>
              </div>
            ):portfolio.map(p=>{
              const lp=prices.find(x=>x.id===p.id)?.price||p.avgPrice;
              const cv=lp*p.qty,cost=p.avgPrice*p.qty,gain=cv-cost;
              return(
            ...
            {txHistory.length===0?(
              ...
              <div key={tx.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"11px 14px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  ...
                  <div>
                    <div style={{fontWeight:700,fontSize:12}}>{tx.side.toUpperCase()} {tx.coinId}</div>
        ...
        {tab==="messages"&&(
          <div style={{padding:14}}>
            ...
            <div style={{fontWeight:700,fontSize:12,color:C.muted,marginBottom:10}}>What would you like help with?</div>
            {[
              {icon:"💳", label:"Withdrawal", desc:"Help with withdrawals"},
              {icon:"💰", label:"Deposit", desc:"Help with deposits"},
              {icon:"🎯", label:"Contact Support", desc:"General support"},
              {icon:"📋", label:"Make a Complaint", desc:"Report an issue"}
            ].map((option,i)=>(
              <div key={i} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:10,cursor:"pointer",display:"flex",gap:12,alignItems:"center"}} onClick={()=>{
                const subject = `Support Request: ${option.label}`;
                const body = `Hello,\n\nI need help with: ${option.label}\n\nDetails:\n[Please describe your issue here]\n\nThank you`;
                window.location.href = `mailto:cryptotrading5@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
              }}>
                <div style={{fontSize:24}}>{option.icon}</div>
                <div>
                  <div style={{fontWeight:700,fontSize:13,color:C.text}}>{option.label}</div>
                  <div style={{fontSize:11,color:C.muted}}>{option.desc}</div>
                </div>
                <span style={{marginLeft:"auto",color:C.accent,fontSize:18}}>›</span>
              </div>
            ))}
            <div style={{background:"#0A1628",border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",marginTop:16,fontSize:11,color:C.muted,textAlign:"center"}}>
...
function AdminPage() {
  ...
  const addBalance = async() => {
    ...
    if(!amount || amount<=0) return setAddErr("Enter valid amount");
    
    try {
      const res = await fetch(`${API_URL}/api/admin-add-balance`, {
        method:"POST",
        headers:{"Content-Type":"application/json", "Authorization":`Bearer ${token}`},
        body:JSON.stringify({email: selectedUser.email, amount})
      });
      const data = await res.json();
      if(!res.ok) return setAddErr(data.error||"Failed");
      
      setUsers(users.map(u=>u.email===selectedUser.email?{...u,balance:u.balance+amount}:u));
    ...
    return (
      <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Inter',system-ui,sans-serif",color:C.text,display:"flex",flexDirection:"column"}}>
        <div style={{background:C.card,borderBottom:`1px solid ${C.gold}`,padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <button onClick={()=>setSelectedUser(null)} style={{background:"none",border:"none",color:C.accent,cursor:"pointer",fontSize:14}}>← Back</button>
        ...
        <div style={{flex:1,overflowY:"auto",padding:20}}>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20,marginBottom:20}}>
            <div style={{fontWeight:700,fontSize:16,marginBottom:8}}>{selectedUser.name}</div>
            <div style={{fontSize:12,color:C.muted,marginBottom:12}}>{selectedUser.email}</div>
            <div style={{fontSize:14,color:C.green,fontWeight:700}}>Balance: ${(selectedUser.balance||0).toLocaleString()}</div>
          </div>

          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:12}}>💰 Add Balance</div>
            <input type="number" value={addAmount} onChange={e=>setAddAmount(e.target.value)} placeholder="Amount ($)" style={{width:"100%",background:"#0A1628",border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"12px 14px",fontSize:14,outline:"none",marginBottom:10,boxSizing:"border-box"}}/>
            {addErr&&<div style={{color:C.red,fontSize:12,marginBottom:10,background:"#2A0A0A",padding:"8px 12px",borderRadius:7}}>{addErr}</div>}
            <button onClick={addBalance} style={{width:"100%",background:C.green,color:"#000",border:"none",borderRadius:10,padding:"12px 0",fontWeight:700,cursor:"pointer",fontSize:14}}>
              Add Balance
            </button>
          </div>
        </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Inter',system-ui,sans-serif",color:C.text,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:20}}>
        ...
        {users.map(u=>(
          <div key={u._id} onClick={()=>setSelectedUser(u)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:10,cursor:"pointer"}}>
